
export const en = {
  header: {
    marketingSupport: "Marketing Support",
    language: "EN",
    supportTooltip: "Marketing Support",
    languageTooltip: "Select Language",
    userTooltip: "User Profile"
  },
  hero: {
    titlePrefix: "Welcome to ",
    titleSuffix: "Redes Comerciales.Sales",
    description: "The 'smart solutions' of our sales machine to get public aid for you or to offer it as a service to your clients.",
    cta: "Intro to our Sales Playbook",
    bullet1: "More sales without expanding internal team",
    bullet2: "Active and aligned partners",
    bullet3: "Full control of performance"
  },
  features: {
    title: "Everything you need to grow",
    subtitle: "One platform, all the tools to take your business to the next level.",
    learnMore: "Learn more"
  },
  featureCards: {
    commercialStrategies: {
      title: "Campaign Generator",
      description: "Offers a complete catalog of outbound marketing campaigns and resources, designed to drive active prospecting and direct lead generation."
    },
    aiMarketing: {
      title: "AI Assistants",
      description: "Includes artificial intelligence assistants that facilitate the adaptation and creation of innovative marketing content for current or future campaigns."
    },
    virtualEvents: {
      title: "Webinars",
      description: "Provides a 2-step structured tool to organize inbound marketing webinars: webinar setup and attendee management."
    },
    commercialAutomation: {
      title: "Automations",
      description: "Lead tracking tool from HubSpot and LinkedIn that centralizes contacts, automatizes tasks, and optimizes the commercial process to improve conversion."
    },
    salesAcademy: {
      title: "Sales Academy",
      description: "Provides resources with virtual assistants to perfect commercial pitches and academic materials to specialize in sales."
    },
    dataRoom: {
      title: "Data Room",
      description: "Centralized repository for sales materials: PPTs, documents, FAQs, templates, and event resources."
    }
  },
  cta: {
    title: "Other Solutions",
    subtitle: "Explore our specialized applications for specific public sector needs.",
    tenderswin_title: "TENDERSWIN.AI",
    tenderswin_description: "Specialized platform for public tenders and government competitions.",
    govsupply_title: "GOVSUPPLY.AI",
    govsupply_description: "Intelligent solutions for public sector providers and state contracting.",
    acceleralia_title: "ACCELERALIA",
    acceleralia_description: "Comprehensive and innovative solution to manage talent and execute projects.",
    grantswin_title: "GRANTSWIN.AI",
    grantswin_description: "The suite that simplifies access to public funding with IA."
  },
  footer: {
    copyright: "© 2026 Redes Comerciales.Sales. All rights reserved.",
    version: "v.1.2",
    solutions: "Solutions",
    platforms: "Our Platforms",
    about: "About Redes Comerciales.Sales",
    aboutDesc: "Transforming every collaborator into an active sales network through AI and commercial automation.",
    contact: "Contact",
    legalSocial: "LEGAL & SOCIAL",
    legalNotice: "Legal Notice",
    cookiesPolicy: "Cookies Policy",
    privacyPolicy: "Privacy Policy"
  },
  modal: {
    title: "In Development",
    description: "Available soon"
  },
  supportModal: {
    title: "Marketing Support",
    nameLabel: "Name",
    namePlaceholder: "Your full name",
    emailLabel: "Email",
    emailPlaceholder: "you@your-email.com",
    messageLabel: "Message",
    messagePlaceholder: "Describe your problem or question...",
    submitButton: "Send Inquiry"
  },
  theme: {
    toggle: "Toggle theme"
  },
  userDropdown: {
    name: "User",
    editProfile: "Edit Profile",
    logout: "Log Out"
  },
  webinarModal: {
    title: "Webinars",
    subtitle: "Access our specialized tools to manage your virtual events.",
    generatorTitle: "Webinar Generator",
    managerTitle: "Attendee Management",
    accessButton: "Access"
  },
  automationModal: {
    title: "Automations",
    subtitle: "Boost your productivity by connecting your favorite tools.",
    hubspotTitle: "HubSpot",
    linkedinTitle: "LinkedIn",
    gmailTitle: "Gmail",
    accessButton: "Access",
    comingSoon: "Coming Soon"
  },
  login: {
    welcomeTitle: "Welcome Back",
    welcomeSubtitle: "Enter your credentials to access Redes Comerciales.Sales",
    emailLabel: "Email Address",
    emailPlaceholder: "you@email.com",
    passwordLabel: "Password",
    passwordPlaceholder: "Your secure password",
    forgotPassword: "Forgot password?",
    submitButton: "Log In",
    submitting: "Verifying...",
    errorGeneric: "Error logging in. Please try again.",
    errorInvalidLogin: "Invalid email or password."
  }
};
